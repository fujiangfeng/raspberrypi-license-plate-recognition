...
from utils.general import (LOGGER, check_file, check_img_size, check_imshow, check_requirements, colorstr,apply_classifier,
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    STN.load_state_dict(torch.load('weights/STNet__iteration_90900.pth',map_location=lambda storage, loc: storage))
    modelc.load_state_dict(torch.load('weights/LPRNet__iteration_90900.pth',map_location=torch.device('cpu')))
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        pred,plat_num= apply_classifier(pred, modelc, STN, im, im0s)
